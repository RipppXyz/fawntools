...
// ============================================================
// 1. IQC FAKE CHAT iPHONE (dengan color picker)
// ============================================================
...
// ============================================================
// 2. IMAGE SEARCH (Cheerio + Unsplash Fallback)
// ============================================================
app.get('/api/imagesearch', async (req, res) => {
  ...
  try {
    // PRIMARY: DuckDuckGo dengan Cheerio
    ...
    // Ambil dari tag img dengan src atau data-src
    $('img').each((i, el) => {
      const src = $(el).attr('src') || $(el).attr('data-src') || $(el).attr('data-original');
      if (src && src.startsWith('http') && !src.includes('duckduckgo.com') && !src.includes('data:image')) {
        // Bersihkan parameter URL
        const clean = src.split('?')[0];
        if (clean.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
          images.add(clean);
        }
      }
    });

    // Ambil dari background-image di style
    ...
    // Kalo DDG gagal, coba Unsplash (gratis, perlu API key)
    if (imageArray.length === 0) {
      console.log('DDG gagal, mencoba Unsplash...');
      const UNSPLASH_KEY = 'YOUR_UNSPLASH_ACCESS_KEY'; // Daftar gratis di unsplash.com/developers
      if (UNSPLASH_KEY !== 'YOUR_UNSPLASH_ACCESS_KEY') {
        ...
        const filteredUnsplash = filterImages(unsplashImages);
        if (filteredUnsplash.length > 0) {
          return res.json({ status: 'success', images: filteredUnsplash.slice(0, 30) });
        }
      }
    }
...
// ============================================================
// 5. ROUTE UTAMA — HTML (dengan color picker)
// ============================================================
app.get('/', (req, res) => {
...
<head>
  ...
  <style>
    /* (CSS sama seperti sebelumnya, tambahan untuk color picker) */
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .iqc-settings input[type="color"] { padding:4px; height:44px; cursor:pointer; }
