...
// ============================================================
// 1. API IQC FAKE CHAT iPHONE (REAL)
// ============================================================
app.get('/api/iqc', async (req, res) => {
  ...
  if (!text) {
    return res.status(400).json({
      ...
      example: '/api/iqc?text=Halo+bang'
    });
  }
  ...
  try {
    ...
    res.status(500).json({
      status: 'error',
      message: 'Gagal generate fake chat, coba lagi nanti'
    });
  }
});

// ============================================================
// 2. API IMAGE SEARCH (YANDEX + SAFESEARCH + FILTER)
// ============================================================
app.get('/api/imagesearch', async (req, res) => {
  ...
  try {
    // Safe search strict + filter konten dewasa
    const url = `https://yandex.com/images/search?text=${encodeURIComponent(q)}&safe=2`;
    ...
    $('img').each((i, el) => {
      ...
      if (src && src.startsWith('http') && !src.includes('yandex.net') && !src.includes('data:image')) {
        ...
        if (cleanUrl.match(/\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i)) {
          images.add(cleanUrl);
        }
      }
    });

    // Ambil dari background-image
    $('[style*="background-image"]').each((i, el) => {
      const style = $(el).attr('style');
      const match = style.match(/url\(['"]?(.*?)['"]?\)/);
      if (match && match[1]) {
        const url = match[1];
        if (url.startsWith('http') && !url.includes('yandex.net')) {
          images.add(url.split('?')[0]);
        }
      }
    });

    // ===== BLACKLIST KATA-KATA PORNO =====
    const blacklist = [
      'adult', 'xxx', 'porn', 'sex', 'nude', '18+', 'bikini',
      'hot', 'sexy', 'erotic', 'porno', 'bokep', 'bugil',
      'telanjang', 'memek', 'kontol', 'ngentot', 'coli'
    ];
    ...
    res.json({ status: 'success', images: imageArray });
...
<body>
  ...
  <div id="app">
    ...
    <!-- PAGE IQC (FAKE CHAT iPHONE) -->
    <div id="page-iqc" class="page active">
      <h2>📱 Fake Chat iPhone</h2>
      <p style="opacity:0.5;font-size:14px;margin:8px 0 16px;">Buat screenshot chat palsu kayak iPhone</p>
      <div class="input-group">
        <input type="text" id="iqc-text" placeholder="Ketik pesan...">
    ...
    <div id="page-imagesearch" class="page">
      <h2>🖼️ Image Search (Yandex · Safe)</h2>
      <div class="input-group">
  ...
  <script>
    ...
    async function generateIQC() {
      ...
      try {
        const response = await fetch('/api/iqc?text=' + encodeURIComponent(text));
        ...
        resultDiv.innerHTML = \`
          <div class="result-box">
            <div class="line"><span class="label">ERROR</span><span class="value highlight">\${err.message}</span></div>
          </div>
        \`;
      } finally {
    ...
    // Auto-load gempa kalo halaman aktif
