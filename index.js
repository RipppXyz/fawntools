...
// ============================================================
// 1. IQC FAKE CHAT (PAKE iqc-canvas - SUPPORT EMOJI!)
// ============================================================
app.get('/api/iqc', async (req, res) => {
    const { text, chatTime = '12.20', replyText, replySender } = req.query;
    if (!text) {
    ...
    try {
        ...
        const result = await generateIQC(text, chatTime, options);
...
// ============================================================
// 2. IMAGE SEARCH (DUCKDUCKGO + YANDEX FALLBACK)
// ============================================================
app.get('/api/imagesearch', async (req, res) => {
    ...
    try {
        ...
        $('[style*="background-image"]').each((i, el) => {
            ...
            if (match && match[1]) {
                const url = match[1];
                if (url.startsWith('http') && !url.includes('duckduckgo.com')) images.add(url.split('?')[0]);
            }
        });
        ...
        if (imageArray.length === 0) {
            const yandexUrl = `https://yandex.com/images/search?text=${encodeURIComponent(q)}&safe=2`;
            const yandexRes = await axios.get(yandexUrl, {
                headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
                timeout: 15000
            });
            const $y = cheerio.load(yandexRes.data);
            const yImages = new Set();
            $y('img').each((i, el) => {
                const src = $(el).attr('src') || $(el).attr('data-src') || $(el).attr('data-original');
                if (src && src.startsWith('http') && !src.includes('yandex.net') && !src.includes('data:image')) {
                    const clean = src.split('?')[0];
                    if (clean.match(/\.(jpg|jpeg|png|gif|webp)$/i)) yImages.add(clean);
                }
            });
            $y('[style*="background-image"]').each((i, el) => {
                const style = $(el).attr('style');
                const match = style.match(/url\(['"]?(.*?)['"]?\)/);
                if (match && match[1]) {
                    const url = match[1];
                    if (url.startsWith('http') && !url.includes('yandex.net')) yImages.add(url.split('?')[0]);
                }
            });
            imageArray = filterImages(Array.from(yImages));
        }
...
<head>
    ...
    <style>
        /* --- GAYA HACKER (tetap sama seperti sebelumnya) --- */
...
<body>
    ...
    <div id="app">
        ...
        <div id="page-iqc" class="page active">
            ...
            <p style="opacity:0.5;font-size:14px;margin:8px 0;">Buat screenshot chat palsu (support emoji!)</p>
            <div class="input-group">
                <input type="text" id="iqc-text" placeholder="Ketik pesan... (bisa pake emoji)">
            ...
            <div class="iqc-settings">
                ...
                <label>Reply Text (opsional) <input type="text" id="iqc-reply" placeholder="Balasan..."></label>
                <label>Reply Sender <input type="text" id="iqc-sender" placeholder="Nama pengirim"></label>
            </div>
    ...
    <script>
        ...
        async function generateIQC() {
            ...
            const chatTime = document.getElementById('iqc-chattime').value || '12.20';
            const replyText = document.getElementById('iqc-reply').value.trim();
            const replySender = document.getElementById('iqc-sender').value.trim();
            ...
            const params = new URLSearchParams({ text, chatTime });
            if (replyText && replySender) {
                params.append('replyText', replyText);
                params.append('replySender', replySender);
            }
